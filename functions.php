...
function getInvoices() {
	...
	if($results) {
		...
		while($row = $results->fetch_assoc()) {
			...
			print '
				    <td><a href="invoice-edit.php?id='.$row["invoice"].'" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-edit" alt="Edit Invoice" title="Edit Invoice" aria-hidden="true"></span></a> 
					
					<a href="invoices/'.$row["invoice"].'.pdf" class="btn btn-info btn-xs" target="_blank"><span alt="View PDF Invoice" title="View PDF Invoice" class="glyphicon glyphicon-download-alt" aria-hidden="true"></span></a> 
					<a data-invoice-id="'.$row['invoice'].'" class="btn btn-danger btn-xs delete-invoice"><span alt="Delete Invoice" title="Delete Invoice" class="glyphicon glyphicon-trash" aria-hidden="true"></span></a></td>
			    </tr>
			';
...
function getAppVersion() {
	...
	$query = "SELECT * FROM inv_system LIMIT 1";

	if ($result = $mysqli->query($query)) {

		$row = mysqli_fetch_assoc($result);

		$row_version = $row['app_version'];	   

	    //var_dump($row);

	  
		echo intval($row_version); 
...
function getInvoiceId() {
	...
	$query = "SELECT MAX(invoice) as ROW_COUNT FROM invoices ORDER BY invoice DESC LIMIT 1";

	if ($result = $mysqli->query($query)) {

		$row = mysqli_fetch_assoc($result);

		$row_cnt = $row['ROW_COUNT'];	   
...
function popProductsList() {
	...
	if ($results) {
		...
		while($row = $results->fetch_assoc()) {
		    ...
		    print '<option value="'. $row['product_price']. '">'.$row["product_code"].' - '.$row["product_name"].' - ' . ' - Price: $' . $row['product_price'] . '</option>';
		}
		echo '</select>';
...
// populate product dropdown for invoice creation
function popCustomersListforProduct() {
		...
		while($row = $results->fetch_assoc()) {
		    ...
		    print '
			    <tr>
				    ...
				    <td>'.$row["email"].'</td>
				    <td>'.$row["phone"].'</td>
				    <td><a href="#" class="btn btn-primary-p btn-xs customer-select1" data-customer-name="'.$row['name'].'" data-customer-email="'.$row['email'].'" data-customer-phone="'.$row['phone'].'" data-customer-address-1="'.$row['address_1'].'" data-customer-address_2="'.$row['address_2'].'" data-customer-town="'.$row['town'].'" data-customer-county="'.$row['county'].'" data-customer-postcode="'.$row['postcode'].'" data-customer-name-ship="'.$row['name_ship'].'" data-customer-address-1-ship="'.$row['address_1_ship'].'" data-customer-address-2-ship="'.$row['address_2_ship'].'" data-customer-town-ship="'.$row['town_ship'].'" data-customer-county-ship="'.$row['county_ship'].'" data-customer-postcode-ship="'.$row['postcode_ship'].'">Select</a></td>
			    </tr>
		    ';
		}
...
// populate invoice dropdown for  creation
function popInvoicesListforCustomer() {
...
function getProducts() {
	...
	if($results) {
		...
		while($row = $results->fetch_assoc()) {
		    ...
		    print '
			    <tr>
					<td style="width: 20px;">'.$row["product_code"].'</td>
