...
class invoicr1 extends FPDF_rotation 
{
	...
	*******************************************************************************/
	function Header()
	{
		...
		try {
		  $positionX = $this->document['w']-$this->margins['l']-$this->margins['r']-max(strtoupper($this->GetStringWidth($this->l['number'])),
			strtoupper($this->GetStringWidth($this->l['date'])),
			strtoupper($this->GetStringWidth($this->l['due'])))-35;
		}
		catch ( Exception $e )
		{
			$positionX = 1;
	...
	// $this->Cell(25,$lineheight,'Invoice No:',0,0,'L');
	    $X1 = $this->GetX();
	    $Y1 = $this->GetY();
	    $this->SetX($X1-10);
		$this->Cell(0-30,$lineheight,'Reference:',0,0,'L');
		...
		$this->Cell(32,$lineheight,'Billing Date:',0,0,'L');	
		$this->SetTextColor(50,50,50);
		$this->SetFont($this->font,'',9);
		...
		if($this->due) 
		{
			...
			$Y1 = $this->GetY();
			$this->SetX($X1-10);
		    $this->Cell(32,$lineheight,iconv("UTF-8", "ISO-8859-1",'Due Date: ' . $this->l['due']),0,0,'L');	
		...
		if($this->PageNo()==1) 
		{
			...
			$this->Ln(5);
			$this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
			$this->SetDrawColor($this->color[0],
			$this->color[1],
			$this->color[2]);
          ...
          //  $this->Cell(5,$lineheight,' ',0,0,'L');
			$this->Cell($width,$lineheight,strtoupper('Our Details'),0,0,'L');
			$X1 = $this->GetX();
			$Y1 = $this->GetY();
			$this->SetX($X1-5);
			$this->Cell($width+25,$lineheight,strtoupper('Customer Details'),0,0,'L');
			$X1 = $this->GetX();
			$Y1 = $this->GetY();
			$this->SetX($X1-30);
			$this->Cell($width,$lineheight,strtoupper('Shipping Details'),0,0,'L'); // ADDED SHIPPING
		//	$this->Ln(7);
		...
		//	$this->Cell(25,$lineheight,' ',0,0,'L');
			...
			$this->Cell($width,$lineheight,strtoupper($this->l['ship']),0,0,'L'); // ADDED SHIPPING
			$this->Ln(7);
			$this->SetLineWidth(0.3);
		//	$this->Line($this->margins['l'], $this->GetY(),$this->margins['l']+$width-10, $this->GetY());
		//	$this->Line($this->margins['l']+$width, $this->GetY(),$this->margins['l']+$width+$width+$width, $this->GetY());
		...
		//	$this->Cell($width,$lineheight,' ',0,0,'L');
			...
			$Y1 = $this->GetY();
			$this->SetX($X1);
			$this->Cell($width,$lineheight,$this->ship[0],0,0,'L'); // ADDED SHIPPING
			...
			for ($i=0; $i<max(count($this->from), count($this->to), count($this->ship)); $i++) { // ADDED SHIPPING
				...
				if ( $i < count($this->from) )
					$this->Cell($width,$lineheight,iconv("UTF-8", "ISO-8859-1",$test),0,0,'L');
				...
				if  (isset($this->to[$i]) )
				    ...
				    $test1 = "";	
				if ( $i < count($this->to) )
					$this->Cell($width,$lineheight,iconv("UTF-8", "ISO-8859-1",$test1),0,0,'L');

				if  (isset($this->ship[$i]) )
				    $test2 = $this->ship[$i];
				else
				    $test2 = "";	
				if ( $i < count($this->ship))
					$this->Cell(0,$lineheight,iconv("UTF-8", "ISO-8859-1",$test2),0,0,'L'); // ADDED SHIPPING
		...
		if(!isset($this->productsEnded)) 
		{
			...
			$this->Cell(20,$lineheight,' ',0,0,'L');
			$this->Ln(12);
			$this->SetFont($this->font,'B',9);
			$X1 = $this->GetX();
			$Y1 = $this->GetY();
			$this->SetX($X1+30);
			$this->Cell($this->columnSpacing,$lineheight,' ',0,0,'L');
		//    $this->Cell($this->columnSpacing,$lineheight,'l',0,0,'L');
		//	$this->firstColumnWidth = 30;
			$this->Cell(1,10,'',0,0,'L',0);
		//	$this->Cell(30,10,iconv("UTF-8", "ISO-8859-1",strtoupper($this->l['product'])),0,0,'L',0);
		    $this->Cell(30,10,iconv("UTF-8", "ISO-8859-1",'Product'),0,0,'L',0);
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			//$this->Cell(20,10,iconv("UTF-8", "ISO-8859-1",strtoupper($this->l['amount'])),0,0,'C',0);
			$this->Cell(25,10,iconv("UTF-8", "ISO-8859-1",'Amount'),0,0,'C',0);
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			//$this->Cell(20,10,iconv("UTF-8", "ISO-8859-1",strtoupper($this->l['amount'])),0,0,'C',0);
			$this->Cell(20,10,iconv("UTF-8", "ISO-8859-1",'GST'),0,0,'C',0);
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			//$this->Cell(20,10,iconv("UTF-8", "ISO-8859-1",strtoupper($this->l['vat'])),0,0,'C',0);
			$this->Cell(15,10,iconv("UTF-8", "ISO-8859-1",'Discount'),0,0,'C',0);
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			//$this->Cell(20,10,iconv("UTF-8", "ISO-8859-1",strtoupper($this->l['price'])),0,0,'C',0);
			$this->Cell(23,10,iconv("UTF-8", "ISO-8859-1",'Quantity'),0,0,'C',0);
	...
	{	
		...
		if($this->items) {
			foreach($this->items as $item) 
			{
			    ...
			    $Y1 = $this->GetY();
			    $this->SetX($X1+3);
				$this->Cell($this->firstColumnWidth+20,$cHeight,iconv("UTF-8", "ISO-8859-1",$item['item']),0,0,'L',1);
	...
	function Footer()
	{
		...
		$this->Cell(0,10,$this->l['page'].' '.$this->PageNo().' of '.$this->l['page_of'].' {nb}',0,0,'R');
	}
